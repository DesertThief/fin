#include <iostream>  // Include the header for cout
#include <string>    // Include the header for to_string
#include "render.h"
...
std::vector<Ray> generatePixelRaysMultisampled(RenderState& state, const Trackball& camera, glm::ivec2 pixel, glm::ivec2 screenResolution)
{
     ...
     std::vector<Ray> rays;
    int numSamples = state.features.numPixelSamples;

    // Calculate the aspect ratio based on screen resolution
    float aspectRatio = static_cast<float>(screenResolution.x) / screenResolution.y;

    // Calculate the field of view from the screen resolution
    float fovy = glm::degrees(2.0f * std::atan(0.5f / camera.distanceFromLookAt()) * aspectRatio);

    for (int i = 0; i < numSamples; ++i) {
        for (int j = 0; j < numSamples; ++j) {
            // Calculate normalized device coordinates for the sub-pixel sample
            float subPixelX = (static_cast<float>(pixel.x) + (i + 0.5f) / numSamples) / screenResolution.x;
            float subPixelY = (static_cast<float>(pixel.y) + (j + 0.5f) / numSamples) / screenResolution.y;

            // Calculate the ray direction based on field of view
            float tanHalfFovy = std::tan(0.5f * glm::radians(fovy));
            float imagePlaneX = (2.0f * subPixelX - 1.0f) * tanHalfFovy * aspectRatio;
            float imagePlaneY = (1.0f - 2.0f * subPixelY) * tanHalfFovy;

            // Construct the ray direction
            glm::vec3 rayDirection = glm::normalize(camera.forward() + imagePlaneX * camera.left() + imagePlaneY * camera.up());

            // Create a ray with the camera position as the origin
            Ray ray;
